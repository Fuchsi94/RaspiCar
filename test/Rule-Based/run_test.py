...
def average_slope_intercept(image, lines):
    ...
    for line in lines:
        ...
        slope, intercept = np.polyfit((int(x1), int(x2)), (int(y1), int(y2)), 1)
...
def display_heading_line(frame, steering_angle, line_color=(0, 0, 255), line_width=5, ):
    ...
    # -20 to -1 degree: turn left
    # 0 degree: going straight
    # 1-20 degree: turn right
    if steering_angle is not 0:
        steering_angle_radian = steering_angle / 180.0 * math.pi
        x1 = int(width / 2)
        y1 = height
        x2 = int(x1 + math.tan(steering_angle_radian) * (height/2))
        y2 = int(height / 2)
...
def get_steering_angle(combo_image, avg_lines):
...
def navigation(image):
    ...
    avg_lines, combo_image = detect_lane_image(image)
    frame, steering_angle = get_steering_angle(combo_image, avg_lines)
...
while cam:
    ret, frame = cam.read()
    if ret:
        steering_image, steering_angle = navigation(frame)
        pwm_signal = int(steering_angle_to_pwm(steering_angle))
        cv2.imshow('frame', steering_image)
        pwm.set_pwm(0, 0, pwm_signal)
    else:
